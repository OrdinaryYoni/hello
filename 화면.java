...
public class 화면 {
	...
	public static void 설명(용사 user) {
		용사 level = user.getStat().getLevel();
		String [] arr = {
				...
				"사냥은 현재 장소에 있는 몬스터와 싸워 경험치를 얻는 선택지,", "채집은 채집 퀘스트를 할 때 쓰는 선택지,","스탯은 자신이 가진 스탯 정보를 알 수 있는 선택지이다.",
				"평소 경험치를 얻을 수 있는 방법은 사냥밖에 없다.", "사냥하는 방법은 간단하다","공격을 선택하면 몬스터를 공격하여 체력을 깎는다.",
				"물론 몬스터도 턴당 1번씩 공격한다.","회복은 자신의 체력을 회복하는 대신 mp를 깎는다.", "여기서 mp란? 스킬을 쓸 수 있게하는 힘이다.",
				"스킬을 쓰는 방법은 행동이다.", "행동을 누르면 스킬을 사용할 수 있는데, 처음에는 휘두르기만 사용할 수 있다.",
				"그러나 나중에 직업이 생기면 더 다양한 스킬을 사용할 수 있다."
				};
		if(level < 21) {
			String [] arr2 = {
					"직업 선택 방법은 에테토스 마을에서 행동을 누르면 5가지 선택지가 나오는데, 그중 모험가길드를 누르면 모험가길드의 선택지가 나온다.",
					"그중 모험가등록을 누르면 직업 선택지가 나와, 그중 하나를 고르면 된다.", "그 외 에테토스 마을의 선택지들을 설명하겠다.",
					"대장간은 무기를 살 수 있다.", "강화는 못하지만 대화는 시도해 볼 순 있다.", "잡화점은 추후 알게될 것이다.", "골목길은 위험한 사람이 항상 있다.",
					"모험가 길드는 직업 선택뿐만아니라 퀘스트를 해결하여 돈을 벌 수 있다.", "왕궁에는 왕이 있다."
			};
		}else if(level < 41) {
			String [] arr2 = {
					"직업 선택 방법은 에테토스 마을에서 행동을 누르면 5가지 선택지가 나오는데, 그중 모험가길드를 누르면 모험가길드의 선택지가 나온다.",
